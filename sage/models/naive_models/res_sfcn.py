...
def conv1x1x1(in_planes, out_planes, stride=1):
    return nn.Conv3d(
        in_planes, out_planes, kernel_size=1, stride=stride, padding=1, bias=False
    )
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        self.downsample = (
            partial(self._downsample_basic_block, planes=planes, stride=stride)
        ...
        for _prev, _next in zip(layers[:-1], layers[1:-1]):
            ...
            if cfg.double:
                feature_extractor.append(
                    ResidualBlock(_prev, _next, batchnorm=batchnorm)
                )
                feature_extractor.append(
                    ResidualBlock(_next, _next, stride=2, batchnorm=batchnorm)
                )

            else:
                feature_extractor.append(
                    ResidualBlock(_prev, _next, stride=2, batchnorm=batchnorm)
                )
