...
class Dinsdale(nn.Module):
    def __init__(self, in_channels, num_classes, filters=1):
        ...
        self.block1 = nn.Sequential(
            BasicConv3d(in_channels, f1), BasicConv3d(f1, f1, max_pool=True)
        )

        self.block2 = nn.Sequential(
            BasicConv3d(f1, f2), BasicConv3d(f2, f2, max_pool=True)
        )

        self.block3 = nn.Sequential(
            BasicConv3d(f2, f3), BasicConv3d(f3, f3, max_pool=True)
        )

        self.block4 = nn.Sequential(
            BasicConv3d(f3, f4), BasicConv3d(f4, f4), BasicConv3d(f4, f4, max_pool=True)
        )

        self.block5 = nn.Sequential(
            BasicConv3d(f4, f5), BasicConv3d(f5, f5, ada_pool=True)
        )

        self.fc = nn.Sequential(nn.Linear(64, 64), nn.Linear(64, num_classes))
