...
from timm.models.layers import Mlp, DropPath, to_2tuple, to_3tuple, trunc_normal_, _assert
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        relative_coords_table = torch.stack(torch.meshgrid([relative_coords_h,
                                                            relative_coords_w,
                                                            relative_coords_d])
                                            ).permute(1, 2, 3, 0).contiguous().unsqueeze(0)  # 1, 2*Wh-1, 2*Ww-1,  2*Wd-1, 3
        ...
        coords_flatten = torch.flatten(coords, start_dim=1)  # 2, Wh*Ww*Wd
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape
        _assert(L == H * W, "input feature has wrong size")
        _assert(H % 2 == 0, f"x size ({H}*{W}*{D}) are not even.")
        _assert(W % 2 == 0, f"x size ({H}*{W}*{D}) are not even.")
        _assert(D % 2 == 0, f"x size ({H}*{W}*{D}) are not even.")
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(
            self, img_size=96, patch_size=4, in_chans=1, num_classes=1, global_pool='avg',
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                input_resolution=(
                    ...
                    self.patch_embed.grid_size[2] // (2 ** i_layer)),
