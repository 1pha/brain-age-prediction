...
class RelevancePropagator:
    ...
    def compute_propagated_relevance(self, layer, relevance):
        ...
        if isinstance(
            layer, (torch.nn.MaxPool1d, torch.nn.MaxPool2d, torch.nn.MaxPool3d)
        ):
    ...
    def linear_inverse(self, m, relevance_in):  # m: layer
        ...
        if self.method == "b-rule":
            ...
            m.in_tensor[:, -in_c:][m.in_tensor[:, -in_c:] < 0] = 0
            m.in_tensor[:, 1 * in_c : 3 * in_c][
                m.in_tensor[:, 1 * in_c : 3 * in_c] > 0
            ] = 0
            ...
            input_relevance[:, : 2 * out_c] *= (1 + self.beta) / norm[:, :out_c].repeat(
                1, 2
            )
            ...
            for i in range(4):
                relevance_out[:, i * in_c : (i + 1) * in_c] = F.linear(
                    ...
                    weight=inv_w[:, i * out_c : (i + 1) * out_c],
                    bias=None,
                )
            ...
            relevance_out = sum(
                [relevance_out[:, i * in_c : (i + 1) * in_c] for i in range(4)]
            )
    ...
    def max_pool_nd_fwd_hook(
        self, m, in_tensor: torch.Tensor, out_tensor: torch.Tensor
    ):
    ...
    def conv_nd_inverse(self, m, relevance_in):
        ...
        if self.method == "b-rule":
            with torch.no_grad():
                ...
                w[-out_c:][w[-out_c:] > 0] = 0
                repeats = np.array(
                    np.ones_like(m.in_tensor.size()).flatten(), dtype=int
                )
                repeats[1] *= 4
                # Repeat across channel dimension (pytorch always has channels first)
                ...
                new_norm[:, :out_c] = norm[:, :out_c] + norm[:, out_c : 2 * out_c]
                new_norm[:, out_c:] = (
                    norm[:, 2 * out_c : 3 * out_c] + norm[:, 3 * out_c :]
                )
                norm = new_norm
                # Some 'rare' neurons only receive either
                # only positive or only negative inputs.
                ...
                input_relevance[:, : 2 * out_c] *= (1 + self.beta) / norm[
                    :, :out_c
                ].repeat(1, 2, *spatial_dims)
                input_relevance[:, 2 * out_c :] *= -self.beta / norm[:, out_c:].repeat(
                    1, 2, *spatial_dims
                )
                ...
                sum_weights = torch.zeros([in_c, in_c * 4, *spatial_dims]).to(
                    self.device
                )
