...
class InnvestigateModel(torch.nn.Module):
    ...
    def __init__(self, the_model, device, lrp_exponent=1, beta=.5, epsilon=1e-6,
                 method="e-rule"):
        """
    ...
    def innvestiage(self, in_tensor=None, rel_for_class=None):
        """
        ...
        with torch.no_grad():
            ...
            for layer in rev_model:
                ...
                r_values_per_layer.append(relevance.to(self.device))
    ...
    def attribute(self, brain: torch.Tensor, **kwargs) -> torch.Tensor:
        rel_for_class = kwargs.get("rel_for_class")
        _, heatmap = self.innvestiage(in_tensor=brain, rel_for_class=rel_for_class)
        return heatmap
