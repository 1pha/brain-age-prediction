...
from sage.models.model_zoo.convnext import LayerNorm
...
class RelevancePropagator:
    ...
    allowed_pass_layers = (torch.nn.BatchNorm1d, torch.nn.BatchNorm2d,
                           ...
                           torch.nn.ReLU, torch.nn.ELU, torch.nn.GELU,
                           torch.nn.Flatten, torch.nn.Identity,
    ...
    def compute_propagated_relevance(self, layer, relevance):
        ...
        elif isinstance(layer, (torch.nn.AvgPool1d, torch.nn.AvgPool2d, torch.nn.AvgPool3d)):
            return self.avg_pool_nd_inverse(layer, relevance).detach()
        ...
        elif isinstance(layer, torch.nn.LayerNorm | LayerNorm):
            return self.layernorm_inverse(layer, relevance).detach()

        elif isinstance(layer, torch.nn.AdaptiveAvgPool3d):
            return self.adaptive_avg_pool3d_inverse(layer, relevance).detach()
    ...
    def get_layer_fwd_hook(self, layer):
        ...
        if isinstance(layer, torch.nn.LayerNorm | LayerNorm):
            return self.layernorm_fwd_hook

        if isinstance(layer, torch.nn.AdaptiveAvgPool3d):
            return self.adaptive_avg_pool3d_fwd_hook
    ...
    def avg_pool_nd_inverse(self, layer, relevance_in):
        # Calculate the relevance for the average pooling layer
        norm = torch.ones_like(layer.out_shape).to(self.device) / (layer.kernel_size ** 3)
        return F.interpolate(relevance_in, size=layer.in_shape[2:], mode='trilinear', align_corners=True) * norm
    ...
    def linear_inverse(self, m, relevance_in):
        ...
        if self.method == "b-rule":
            ...
            input_relevance = relevance_in.to(self.device).squeeze(dim=-1).repeat(1, 4)
            input_relevance[:, :2*out_c] *= (1+self.beta)/norm[:, :out_c].repeat(1, 2)
            input_relevance[:, 2*out_c:] *= -self.beta/norm[:, out_c:].repeat(1, 2)
            inv_w = w.t()
            relevance_out = torch.zeros_like(m.in_tensor, device=self.device)
            for i in range(4):
    ...
    def layernorm_inverse(self, layer, relevance):
        """
        Compute the backward pass for the LayerNorm layer.

        Args:
            layer: The LayerNorm layer to be reverted.
            relevance: The incoming relevance from higher up in the network.

        Returns:
            The propagated relevance for the LayerNorm layer.
        """
        mean = layer.in_tensor.mean(dim=-1, keepdim=True)
        std = layer.in_tensor.std(dim=-1, keepdim=True, unbiased=False)
        normalized_input = (layer.in_tensor - mean) / (std + 1e-9)
        
        relevance_out = relevance / (std + 1e-9)
        
        return relevance_out

    @module_tracker
    def layernorm_fwd_hook(self, m, in_tensor: torch.Tensor, out_tensor: torch.Tensor):
        setattr(m, "in_tensor", in_tensor[0])
        return
            ...
            with torch.no_grad():
                ...
                for i in range(4):
                    tmp_result = inv_conv_nd(
                        ...
                        groups=m.groups)
                    result = torch.zeros_like(relevance_out[:, i*in_c:(i+1)*in_c], device=self.device)
    ...
    def adaptive_avg_pool3d_inverse(self, layer, relevance):
        """
        Compute the backward pass for the AdaptiveAvgPool3d layer.

        Args:
            layer: The AdaptiveAvgPool3d layer to be reverted.
            relevance: The incoming relevance from higher up in the network.

        Returns:
            The propagated relevance for the AdaptiveAvgPool3d layer.
        """
        input_shape = layer.in_shape
        output_shape = relevance.shape

        # Ensure the relevance tensor has the same number of dimensions as the input
        if len(output_shape) < len(input_shape):
            for _ in range(len(input_shape) - len(output_shape)):
                relevance = relevance.unsqueeze(-1)
        factor = np.prod([i / o for i, o in zip(input_shape[2:], output_shape[2:])])
        
        relevance_out = F.interpolate(relevance, size=input_shape[2:], mode='trilinear', align_corners=True)
        relevance_out *= factor

        return relevance_out

    @module_tracker
    def adaptive_avg_pool3d_fwd_hook(self, m, in_tensor: torch.Tensor, out_tensor: torch.Tensor):
